...
pub(crate) fn generate_terrain_chunk(
    ...
    let mut octave_offsets: [cgmath::Vector2<i32>; NOISE_OCTAVES as usize] =
        [Vector2 { x: 0, y: 0 }; NOISE_OCTAVES as usize];
        ...
        for x in 0..width {
            ...
            let vertex_y_value = if x >= path_left_edge && x <= path_right_edge {
                ...
                for octave in 0..NOISE_OCTAVES as u32 {
                    ...
                    let sample_x = (((world_x as f64)
                        + (octave_offsets[octave as usize].x as f64))
                        / NOISE_SCALE)
                        * noise_frequency;
                    let sample_y = (((world_z as f64)
                        + (octave_offsets[octave as usize].y as f64))
                        / NOISE_SCALE)
                        * noise_frequency;
            ...
            if x < width - 1 && z < length {
                ...
                let is_in_canyon = (x >= path_left_edge + 1 && x <= path_right_edge - 1)
